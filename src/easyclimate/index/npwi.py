...
def cal_NPWI_monsoon_onset(
    ...
    NPWI_overthresh_conti3 = NPWI_overthresh.rolling(time = consecutive_days, center=True).sum()
    NPWI_overthresh_over3 = (NPWI_overthresh_conti3 > (consecutive_days - 1)).astype(int)
    
    # n out of 9 grid cells must meet the above conditions
    monsoonindex = NPWI_overthresh_over3.rolling({lon_dim: 3, lat_dim: 3}, center=True).sum()
...
def cal_NPWI_monsoon_detreat(
    ...
    dateofyear_dataarray_overonset = dateofyear_dataarray.where((dateofyear_dataarray > monsoon_onset_date))
