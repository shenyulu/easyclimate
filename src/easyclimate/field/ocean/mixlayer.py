...
def calc_MLD_depth_weighted(
    ...
    Parameters
    ----------
    seawater_temperature_data: :py:class:`xarray.DataArray<xarray.DataArray>` or :py:class:`xarray.Dataset<xarray.Dataset>` (:math:`\\mathrm{^\circ C}`).
        Vertical seawater temperature data.
    ...
    depth_expanded = xr.broadcast(seawater_temperature_data[depth_dim], seawater_temperature_data)[0]
...
def calc_MLD_temper_tendency(
    ...
    temper_mld_vertical = seawater_temperature_anomaly_data.where(seawater_temperature_anomaly_data[depth_dim] <= mld_expanded).compute()
...
def get_temper_within_MLD(
    ...
    """
    return get_data_within_MLD(data_input = seawater_temperature_data, mixed_layer_depth = mixed_layer_depth, depth_dim = depth_dim)
...
def get_temper_average_within_MLD(
    ...
    """
    return get_data_average_within_MLD(data_input = seawater_temperature_data, mixed_layer_depth = mixed_layer_depth, depth_weight = depth_weight, depth_dim = depth_dim)
...
def calc_MLD_average_horizontal_advection(
    ...
    Parameters
    ----------
    u_monthly_data: :py:class:`xarray.DataArray<xarray.DataArray>`.
        The monthly ocean current data.
    v_monthly_data: :py:class:`xarray.DataArray<xarray.DataArray>`.
    ...
    u_advection = calc_u_advection(u_data = u_monthly_data, temper_data = seawater_temperature_data, 
...
def calc_MLD_average_vertical_advection(
    ...
    w_advection = w_monthly_data *calc_gradient(seawater_temperature_data, dim = depth_dim) *2626560 *depth_weight
...
def calc_ocean_surface_heat_flux(
    ...
    mixed_layer_depth: :py:class:`xarray.DataArray<xarray.DataArray>`(:math:`\\mathrm{m}`).
        The mixed layer depth.
    rho_0: :py:class:`float<python.float>`, default: `1000` (:math:`\\mathrm{kg \cdot m^{-3}}`).
        The density of water.
    c_p: :py:class:`float<python.float>`, default: `4000` (:math:`\\mathrm{J \cdot kg \cdot K^{-1}}`).
