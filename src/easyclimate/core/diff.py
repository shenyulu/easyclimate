...
def calc_gradient(
    ...
    def _calc_gradient(data_input, dim, varargs, edge_order) -> xr.DataArray: 
        ...
        data_gradient = np.gradient(data_input.data, varargs, axis = dim_index, edge_order = edge_order)
...
def calc_delta_pressure(
    ...
    data_input = data_input.assign_coords({vertical_dim: data_input[vertical_dim] *vertical_dim_base})

    # Change the surface pressure data unit to `Pa`
    surface_pressure_data = transfer_data_units(surface_pressure_data, surface_pressure_data_units, 'Pa')
    ...
    # top level
    delta_pressure[{vertical_dim: -1}] = (pressure_lev[{vertical_dim: -1}] + pressure_lev[{vertical_dim: -2}]) / 2 - pressure_top
    # bottom level
    delta_pressure[{vertical_dim: 0}] = surface_pressure - (pressure_lev[{vertical_dim: 0}] + pressure_lev[{vertical_dim: 1}]) / 2
...
def calc_top2surface_integral(
    ...
    if method == 'Boer-vibeta':   
        ...
        # Change the vertical coordinate unit to `Pa`
        vertical_dim_array = transfer_data_units(data_input[vertical_dim], vertical_dim_units, 'Pa')
        ...
        pressure2surface_data = data_input.where(level_expanded <= surface_pressure_data)
        pressure2surface_data = pressure2surface_data.fillna(0)
        fint = calc_p_integral(pressure2surface_data, vertical_dim = vertical_dim, normalize = normalize)
        ...
        dp = calc_delta_pressure(data_input, surface_pressure_data, vertical_dim,
                                surface_pressure_data_units, vertical_dim_units)
        ...
        raise ValueError("The parameter `method` should be `'Boer-vibeta'` or `'Trenberth-vibeta'`.")
...
def calc_laplacian(
    ...
    if spherical_coord == True:
        term3 = 1 /R *calc_gradient(data_input, dim = lat_dim) /dy *np.tan(transfer_deg2rad(data_input[lat_dim]))
...
def calc_divergence(
    ...
    if u_data.shape != v_data.shape:
        raise ValueError('`u`, `v` shape must be same! However, the u shape is ', u_data.shape, ', and v shape is ', v_data.shape, '.')
...
def calc_geostrophic_wind_vorticity(
    ...
    """
    geostrophic_wind = calc_geostrophic_wind(z_data, lon_dim = lon_dim, lat_dim = lat_dim, 
                                   omega = omega, g = g, R = R)
    ug, vg = geostrophic_wind['ug'], geostrophic_wind['vg']
    vor_g = calc_vorticity(ug, vg, spherical_coord = spherical_coord, 
                           lon_dim = lon_dim, lat_dim = lat_dim, R = R)
    return vor_g
...
def calc_horizontal_water_flux(
    ...
    """
    water_flux = xr.Dataset(data_vars = {'qu': specific_humidity_data *u_data /g, 'qv': specific_humidity_data *v_data /g})
    return water_flux
...
def calc_water_flux_top2surface_integral(
    ...
    water_flux_single_layer = calc_horizontal_water_flux(specific_humidity_data, u_data, v_data, g = g)
    ...
    qu_top2surface_integral = calc_top2surface_integral(
        ...
        surface_pressure_data = surface_pressure_data, surface_pressure_data_units = surface_pressure_data_units,
    ...
    quv_top2surface_integral = xr.Dataset(data_vars = {'qu': qu_top2surface_integral, 'qv': qv_top2surface_integral})
...
def calc_divergence_watervaporflux(
    ...
    """
    specific_humidity = transfer_data_units(specific_humidity_data, specific_humidity_units, 'kg/kg')
    divergence_watervaporflux = (1 /g) *calc_divergence(u_data = specific_humidity *u_data, v_data = specific_humidity *v_data, 
...
def calc_divergence_watervaporflux_top2surface_integral(
    ...
    divergence_watervaporflux_single_layer = calc_divergence_watervaporflux(
        specific_humidity, u_data, v_data, spherical_coord = spherical_coord, specific_humidity_units = specific_humidity_units, lon_dim = lon_dim, lat_dim = lat_dim, g = g, R = R)
    
    # Calculate the whole layer water vapor flux divergence
    divergence_watervaporflux_single_layer_top2surface_integral = calc_top2surface_integral(
