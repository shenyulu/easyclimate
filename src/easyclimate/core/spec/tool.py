...
from ...backend import spharm, spectral_tool_rs, Spharmt_rs
import xarray as xr
import numpy as np
...
def calc_gaussian_latitudes_rs(nlat: int):
    """
    ...
    """
    lats, wts = spectral_tool_rs.gaussian_lats_wts_rs(nlat)
...
def calc_geodesic_points_rs(m: int):
    """
    Calculate the lat/lon values of the points on the surface of the sphere
    corresponding to a twenty-sided (icosahedral) geodesic by Rust backend.
    ...
    """
    lats, lons = spectral_tool_rs.getgeodesicpts_rs(m)
...
def calc_spherical_harmonic_coefficients_rs(ntrunc: int):
    """
    ...
    """
    indxm, indxn = spectral_tool_rs.getspecindx(ntrunc)
...
def calc_legendre_functions_rs(lat: float, ntrunc: int) -> xr.DataArray:
    """
    ...
    """
    pnm = spectral_tool_rs.legendre_rs(lat, ntrunc)
    pnm_array = xr.DataArray(pnm, name="pnm")
...
def transfer_grid2spectral_transform_rs(
    grid_data: xr.DataArray,
    grid_data_type: Literal["regular", "gaussian"],
    lon_dim: str = "lon",
    lat_dim: str = "lat",
    ...
    # Initialize spherical harmonic transform object
    sph = Spharmt_rs(
        nlon, nlat, gridtype=grid_data_type, transform_backend=transform_backend
    )
...
def transfer_spectral_transform2grid_rs(
    spec_data: xr.DataArray,
    nlon: int,
    nlat: int,
    grid_data_type: Literal["regular", "gaussian"],
