...
def interp_point2mesh(data, var_name, lon_dim_name = 'lon', lat_dim_name = 'lat',
                      point=[-9.,47.], grid_x = 12, grid_y = 12, resolution = 32.0, 
                      sigma = 1.0, method='optimized_convolution',
    ...
    step = 1.0 / resolution
    x0 = np.asarray([point[0], point[1]], dtype=np.float64)
    ...
    field = barnes(lon_lat_data, qff_values, sigma, x0, step, size, method = method, num_iter = num_iter, min_weight = min_weight)

    field_dataarray = xr.DataArray(field, dims = ('lon', 'lat'), coords = {lon_dim_name: gridX, lat_dim_name: gridY}, name = var_name)
    return field_dataarray

def interp_point2mesh_S2(data, var_name, lon_dim_name = 'lon', lat_dim_name = 'lat',
                      point=[-9.,47.], grid_x = 12, grid_y = 12, resolution = 32.0, sigma = 1.0,
                      method='optimized_convolution_S2', num_iter=4, resample=True,
):
    """
    Computes the Barnes interpolation for observation values `var_name` taken at sample
    points `data` using Gaussian weights for the width parameter `sigma`.
    ...
    # calculate Barnes interpolation from fastbarnes import interpolation
    field = barnes_S2(lon_lat_data, qff_values, sigma, x0, step, size, method=method, num_iter=num_iter, resample=resample)
