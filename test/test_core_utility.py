...
data_time_series = xr.DataArray(
    np.array([[57413.867, 57411.96 ],
       ...
       [57512.867, 57516.676],
       [57421.48 , 57427.195],
       [57259.65 , 57261.555]]), dims = ('time', 'lon'),
       coords = {'time': pd.date_range('1982-01-01', periods = 30, freq = 'M'), 'lon': np.array([100.125, 101.25 ])}
)

ds1 = xr.DataArray(
    np.array([3e-5, 5.6e-5, 7e-5]),
    dims = 'lat',
    coords = {'lat': np.array([10, -20, 30])}
)
ds2 = xr.DataArray(
    np.array([3e-5, 5.4e-5, 7.3e-5]),
    dims = 'lon',
    coords = {'lon': np.array([0, 320, 78])}
)
ds3 = xr.DataArray(
...
def test_transfer_int2datetime():
    ...
    refer_data = np.array(['2054-01-01T00:00:00.000000000', '2061-01-01T00:00:00.000000000',
...
def test_split_datetime2yearday():
    result_data = ecl.utility.split_datetime2yearday(data_time_series.isel(lon = 0)).sel(year = 1983).data
    refer_data = np.array([57568.082, 57625.195, 57653.758, 57697.547, 57613.773, 57501.445,
       57446.23 , 57440.523, 57438.617, 57387.21 , 57507.156, 57469.08 ])
...
def test_transfer_monmean2everymonthmean():
    ...
    refer_data = np.array([57443.694     , 57467.17633333, 57543.966     , 57580.77333333,
       57533.17533333, 57411.96166667, 57377.69      , 57391.9715    ,
       57423.3845    , 57414.818     , 57492.878     , 57527.1475    ,
       57443.694     , 57467.17633333, 57543.966     , 57580.77333333,
       57533.17533333, 57411.96166667, 57377.69      , 57391.9715    ,
       57423.3845    , 57414.818     , 57492.878     , 57527.1475    ,
       57443.694     , 57467.17633333, 57543.966     , 57580.77333333,
       57533.17533333, 57411.96166667])
    assert np.isclose(result_data, refer_data).all()
...
def test_sort_ascending_latlon_coordinates():
    result_data1 = ecl.utility.sort_ascending_latlon_coordinates(ds1, lat_dim = 'lat', lon_dim=None).data    
    result_data2 = ecl.utility.sort_ascending_latlon_coordinates(ds2, lon_dim = 'lon', lat_dim=None).data
    result_data3 = ecl.utility.sort_ascending_latlon_coordinates(ds3, lon_dim = 'lon', lat_dim='lat').data.flatten()
    refer_data1 = np.array([5.6e-05, 3.0e-05, 7.0e-05])
    refer_data2 = np.array([3.0e-05, 7.3e-05, 5.4e-05])
    refer_data3 = np.array([-7.55e+01,  5.20e+00,  5.40e-05,  3.00e-05])
    assert np.isclose(result_data1, refer_data1).all()
    assert np.isclose(result_data2, refer_data2).all()
...
def test_generate_dataset_dispatcher():
    ...
    def func(
        testdata: xr.DataArray | xr.Dataset
...
def test_transfer_xarray_lon_from180TO360():
    result_data = ecl.utility.transfer_xarray_lon_from180TO360(ds4, lon_dim = 'lon').lon.data
    refer_data = np.array([120, 182, 310])
    assert np.isclose(result_data, refer_data).all()

def test_transfer_xarray_lon_from360TO180():
    result_data = ecl.utility.transfer_xarray_lon_from360TO180(ds5, lon_dim = 'lon').lon.data
