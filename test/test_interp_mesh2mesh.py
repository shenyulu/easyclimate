...
data_output = xr.open_dataset(str(Path(TEST_DATA_PATH, 'test_output_interp_mesh2mesh.nc')))
...
def test_interp_mesh2mesh2():
    target_grid = xr.DataArray(
        dims = ('lat', 'lon', 'time'),
        coords = {'lon': np.linspace(0, 357.5, 72), 'lat': np.linspace(-90, 90, 36), 'time': np.array([1, 2])}
    )
    with pytest.raises(ValueError):
        ecl.interp.interp_mesh2mesh(data_u, target_grid, lon_dim = 'lon', lat_dim = 'lat', method = 'linear')
        assert 1 == 1
...
def test_interp_mesh2mesh4():
    ...
    result_data = ecl.interp.interp_mesh2mesh(data_u, target_grid, lon_dim = 'lon', lat_dim = 'lat', method = 'linear').data.flatten()
...
def test_interp_mesh2mesh4():
    ...
    result_data = ecl.interp.interp_mesh2mesh(data_u1, target_grid, lon_dim = 'lon', lat_dim = 'lat', method = 'linear').data.flatten()
    refer_data = data_output_linear.data.flatten()
    assert np.isclose(result_data, refer_data).all()