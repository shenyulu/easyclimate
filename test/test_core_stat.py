...
def test_calc_linregress_spatial_datatree():
   data = sic_data_Barents_Sea_12.sel(lon = slice(34.5, 36.5), lat = slice(78.5, 80.5)).to_dataset(name = 'sic')
...
def test_calc_theilslopes_spatial():
   result_data = ecl.calc_theilslopes_spatial(sic_data_Barents_Sea_12.sel(lon = slice(34.5, 36.5), lat = slice(78.5, 80.5)), dim = 'time').compute()
   ...
   refer_data1 = np.array([-0.00999999, -0.00999999, -0.01      , -0.0035    , -0.00296296,
       -0.00258064, -0.0025    , -0.00179487, -0.001875  ])
   refer_data2 = np.array([1.10499978, 1.09499979, 1.08499995, 1.00675   , 0.99574073,
       0.99290321, 0.91125007, 0.92179486, 0.93343744])
   refer_data3 = np.array([-0.01833333, -0.018     , -0.019     , -0.00853659, -0.0075    ,
       -0.00736842, -0.005625  , -0.00416667, -0.00333334])
   refer_data4 = np.array([-0.003     , -0.00285714, -0.00333333, -0.00090909, -0.00058823,
       -0.00058823,  0.        ,  0.        , -0.000625  ])
