...
def test_calc_TaylorDiagrams_metadata1():
    ...
    refer_data2 = np.array([1., 0.40462089, 2.05647001])
    refer_data3 = np.array([ 1., -0.42939816,  0.98408606])
    refer_data4 = np.array([0., 1.22931078, 1.08700596])
...
def test_calc_TaylorDiagrams_metadata2():
    ...
    refer_data3 = np.array([ 1., -0.38222542,  0.97689714])
    refer_data4 = np.array([0., 2.96182156, 2.63594059])
...
def test_draw_TaylorDiagrams_base3():
    fig, ax = plt.subplots(subplot_kw = {'projection': 'polar'})
    ecl.plot.draw_TaylorDiagrams_base(ax = ax, std_max = 2.5, half_circle = True, normalized = False)
    return fig

@pytest.mark.mpl_image_compare
def test_draw_TaylorDiagrams_base4():
    fig, ax = plt.subplots(subplot_kw = {'projection': 'polar'})
    ecl.plot.draw_TaylorDiagrams_base(ax = ax, std_max = 2.5, half_circle = False, normalized = False)
    return fig

@pytest.mark.mpl_image_compare
def test_draw_TaylorDiagrams_base5():
    fig, ax = plt.subplots(subplot_kw = {'projection': 'polar'})
    ecl.plot.draw_TaylorDiagrams_base(ax = ax, std_max = 2.5, half_circle = True, normalized = True)
    return fig
...
def test_draw_TaylorDiagrams_base9():
    fig, ax = plt.subplots(subplot_kw = {'projection': 'polar'})
    with pytest.raises(ValueError):
        ecl.plot.draw_TaylorDiagrams_base(ax = ax, std_min = -1, half_circle = "This is a sample!")
        assert 1 == 1
